...
public class TreatJPanel extends javax.swing.JPanel {
    ...
    private RescueRecord rescueRecord;
    private TreatmentRecord treatment;
    
    public TreatJPanel(JPanel container, Enterprise enterprise, UserAccount userAccount, RescueRecord record, TreatmentRecord treatment) {
        initComponents();
    ...
    public void populateAnimal(){
        ...
        row[0] = rescueRecord.getAnimal().getAnimalId();
        row[1] = rescueRecord.getAnimal().getType();
        row[2] = rescueRecord.getAnimal().getColor();
        model.addRow(row);
    }
    ...
    private void btnFinishActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnFinishActionPerformed
        ...
        rescueRecord.getAnimal().setAge(age);
        if(rescueRecord.getAnimal() instanceof Cat) {
            if(rbtnNone.isSelected() == true) {
                ((Cat)rescueRecord.getAnimal()).setBreed("None");
            }
            else {
                ((Cat)rescueRecord.getAnimal()).setBreed(txtBreed.getText());
            }
        }
        if(rescueRecord.getAnimal() instanceof Dog) {
            if(rbtnNone.isSelected() == true) {
                ((Dog)rescueRecord.getAnimal()).setBreed("None");
            }
            else {
                ((Dog)rescueRecord.getAnimal()).setBreed(txtBreed.getText());
            }
        }
        
        if(rbtnHealthy.isSelected() == true) {
            rescueRecord.setHealthy(true);
        }
        else {
            rescueRecord.setHealthy(false);
        }
        ...
        rescueRecord.getAnimal().setIsSterilized(sterilized);
        System.out.println("-----------------------------------");
        System.out.println(enterprise.getCageCellDir().getCageCellList().size());
        for(CageCell c: enterprise.getCageCellDir().getCageCellList()) {
            System.out.println(c.getStatus());
        }
        for(CageCell c: enterprise.getCageCellDir().getCageCellList()) {
            if(c.getStatus().equals(CageCell.AVAIL_STATUS)) {
                c.setAnimal(rescueRecord.getAnimal());
                c.setStatus(CageCell.OCCUPIED_STATUS);
                isFull = false;
                break;
            }
        }
